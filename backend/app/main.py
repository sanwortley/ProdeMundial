...
# Migrate: add ganador column to partidos (knockout winner, incl. ET/penalties)
with engine.connect() as conn:
    try:
        conn.execute(text("ALTER TABLE partidos ADD COLUMN ganador VARCHAR"))
        conn.commit()
        logger.info("Added ganador column to partidos table (migration)")
    except Exception:
        pass

# Migrate: add ganador_predicho column to predicciones (playoff winner prediction)
with engine.connect() as conn:
    try:
        conn.execute(text("ALTER TABLE predicciones ADD COLUMN ganador_predicho VARCHAR"))
        conn.commit()
